...
parser = subparser
 ...
 where
  start       = startApp <$> databaseFileOption <*> isProdOption
...
isProdOption = switch
  (short 'p' <> long "production" <> help "Run the app in production mode")

main :: IO ()
main = join $ execParser opts
 where
  opts = info (parser <**> helper) desc
  desc = fullDesc <> progDesc "Manage your patients with ease" <> header
    "Hypo API server"
